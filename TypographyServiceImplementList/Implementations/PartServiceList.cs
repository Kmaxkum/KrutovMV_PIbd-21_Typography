...
namespace TypographyServiceImplementList.Implementations
{
    public class PartServiceList : IPartService
    {
        ...
        {
            List<PartViewModel> result = source.Parts.Select(rec => new
PartViewModel
            {
                Id = rec.Id,
        ...
        {
            Part element = source.Parts.FirstOrDefault(rec => rec.PartName
== model.PartName);
            if (element != null)
            {
                throw new Exception("Уже есть компонент с таким названием");
            }
            int maxId = source.Parts.Count > 0 ? source.Parts.Max(rec =>
           rec.Id) : 0;
