...
{
    public class EditionServiceDB : IEditionService
    {
        private TypographyDbContext context;
        public EditionServiceDB(TypographyDbContext context)
        {
        ...
        public void SaveItemPrice(EditionBindingModel model)
        {
            ...
            try
            {
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].ItemName;
                ...
                document.SaveAs(model.FilePath, ref fileFormat, ref missing, 
            ...
            return context.Storages
                ...
                .GroupJoin(
                    context.StorageParts
                        ...
                        .ToList(),
                    storage => storage,
                    storagePart => storagePart.Storages,
                    (stock, storagePartList) =>
            new StorageLoadViewModel
            {
                StorageName = stock.StorageName,
                TotalCnt = storagePartList.Sum(r => r.Cnt),
                Parts = storagePartList.Select(r => new Tuple<string, int>(r.Parts.PartName, r.Cnt))
            })
            .ToList();
        }
        public void SaveStoragesLoad(EditionBindingModel model)
        {
            ...
            try
            {
                ...
                if (File.Exists(model.FilePath))
                {
                    excel.Workbooks.Open(model.FilePath, Type.Missing, Type.Missing, Type.Missing,
                ...
                else
                {
                    ...
                    excel.Workbooks[1].SaveAs(model.FilePath, XlFileFormat.xlExcel8, Type.Missing,
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        if (elem.Parts.Count() > 0)
                        {
                            ...
                            var excelBorder = excelworksheet.get_Range(excelcells, excelcells.get_Offset(elem.Parts.Count() - 1, 1));
                        ...
                        excelcells.Value2 = elem.TotalCnt;
                        excelcells.Font.Bold = true;
                    }
                }
                //сохраняем
        ...
        public List<CustomerBookingViewModel> GetCustomerBookings(EditionBindingModel model)
        {
        ...
        public void SaveCustomerBookings(EditionBindingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].Cnt.ToString(), fontForCells));
